import React from "react"
import { useDispatch } from "react-redux"
import { setSortType } from "../redux/filter/slice"
import { Sort as SortType, SortPropertyEnum } from "../redux/filter/types"

type SortItem = {
	name: string
	sort: SortPropertyEnum
}

type SortPopupProps = {
	value: SortType
}

export const sortList: SortItem[] = [
	{ name: "популярности ↓", sort: SortPropertyEnum.RATING_DESC },
	{ name: "популярности ↑", sort: SortPropertyEnum.RATING_ASC },
	{ name: "цене ↓", sort: SortPropertyEnum.PRICE_DESC },
	{ name: "цене ↑", sort: SortPropertyEnum.PRICE_ASC },
	{ name: "алфавиту ↓", sort: SortPropertyEnum.TITLE_ASC },
	{ name: "алфавиту ↑", sort: SortPropertyEnum.TITLE_DESC },
]

export const Sort: React.FC<SortPopupProps> = React.memo(({ value }) => {
	const dispatch = useDispatch()
	const sortRef = React.useRef<HTMLDivElement>(null)

	const [hidePopup, setHidePopup] = React.useState(true)

	React.useEffect(() => {
		const handleClick = (event: MouseEvent) => {
			const _event = event as MouseEvent & { path: Node[] }
			if (sortRef.current !== null && !_event.path.includes(sortRef.current)) {
				setHidePopup(true)
			}
		}
		document.body.addEventListener("click", handleClick)

		return () => {
			document.body.removeEventListener("click", handleClick)
		}
	}, [])

	const changeActivePopup = (popup: SortItem) => {
		dispatch(setSortType(popup))
		setHidePopup(true)
	}

	return (
		<div ref={sortRef} className="sort">
			<div
				onClick={() => setHidePopup(hidePopup => !hidePopup)}
				className="sort__label"
			>
				<svg
					style={{ transform: !hidePopup ? "rotateX(180deg)" : undefined }}
					width="10"
					height="6"
					viewBox="0 0 10 6"
					fill="none"
					xmlns="http://www.w3.org/2000/svg"
				>
					<path
						d="M10 5C10 5.16927 9.93815 5.31576 9.81445 5.43945C9.69075 5.56315 9.54427 5.625 9.375 5.625H0.625C0.455729 5.625 0.309245 5.56315 0.185547 5.43945C0.061849 5.31576 0 5.16927 0 5C0 4.83073 0.061849 4.68424 0.185547 4.56055L4.56055 0.185547C4.68424 0.061849 4.83073 0 5 0C5.16927 0 5.31576 0.061849 5.43945 0.185547L9.81445 4.56055C9.93815 4.68424 10 4.83073 10 5Z"
						fill="#2C2C2C"
					/>
				</svg>
				<b>Сортировка по:</b>
				<span>{value.name}</span>
			</div>
			{!hidePopup && (
				<div className="sort__popup">
					<ul>
						{sortList.map((item, index) => (
							<li
								onClick={() => changeActivePopup(sortList[index])}
								className={value.name === item.name ? "active" : undefined}
								key={index}
							>
								{item.name}
							</li>
						))}
					</ul>
				</div>
			)}
		</div>
	)
})
